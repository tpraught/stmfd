...
//find Gordy Dehmer's doc and delete it.
db.Roster
  .remove({})
  ...
  .then(data => {
	 ...
	 deleteRecordId = data[0]._id;  //save the id of the document that will be deleted.
  })
  .then(() => db.Roster.remove({"_id":mongoose.Types.ObjectId(deleteRecordId)})) //delete doc 
  .then(data => {
	  process.exit(0);
  })
  .catch(err => {
